...
export function AssetForm({
  ...
  async function submit(values: FormValues) {
    ...
    const finalValue =
      qty && prc ? Math.round(qty * prc * 100) / 100 : values.value ? Number(values.value) : 0;
  ...
  const inputClass =
    'w-full border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 text-sm bg-white dark:bg-gray-800 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-brand';
  ...
  return (
    <form onSubmit={handleSubmit(submit)} className="space-y-4">
      ...
      {selectedType === 'etf' && (
        <div>
          <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
            {t('assetForm.ticker' as any)}
          </label>
          <input
            {...register('ticker')}
            className={inputClass}
            placeholder={t('assetForm.tickerPlaceholder' as any)}
          />
        </div>
      )}
      ...
      {selectedType === 'crypto' && (
        <div>
          <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
            {t('assetForm.coinId' as any)}
          </label>
          <input
            {...register('coinId')}
            className={inputClass}
            placeholder={t('assetForm.coinIdPlaceholder' as any)}
          />
        </div>
      )}
      ...
      {selectedType === 'gold' && (
        <div>
          <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
            {t('assetForm.goldUnit' as any)}
          </label>
          <select {...register('goldUnit')} className={inputClass}>
            {GOLD_UNITS.map((u) => (
              <option key={u} value={u}>
                {t(`assetForm.${u === 'g' ? 'grams' : 'troyOunces'}` as any)}
              </option>
            ))}
          </select>
        </div>
            ...
            <div className="rounded-lg bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 px-3 py-2">
              <p className="text-sm text-gray-700 dark:text-gray-300">
                ...
                <span className="font-semibold">
                  {formatCurrency(computedValue, watchedCurrency ?? 'EUR')}
                </span>
              </p>
            </div>
          )}
