    ...
    <Modal title={`History — ${assetName}`} onClose={onClose}>
      <form onSubmit={handleAdd} className="flex flex-wrap gap-2 mb-5">
        ...
        <input
          ...
          min={0}
          step="0.01"
          className="flex-1 min-w-[120px] border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand"
          required
        />
        <button
          type="submit"
        ...
        <div className="divide-y divide-gray-100">
          {[...snapshots].reverse().map((s) => (
            <div key={s.id} className="flex items-center justify-between py-2 text-sm">
              <span className="text-gray-500">{s.capturedAt.slice(0, 7)}</span>
