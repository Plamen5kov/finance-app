  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
        {IMPORT_TYPES.map((type) => (
          ...
          >
            <div className="flex items-start gap-3">
              ...
              <div>
                <h3 className="font-medium text-gray-900 dark:text-gray-100">{t(type.nameKey)}</h3>
                <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">{t(type.descKey)}</p>
              </div>
            </div>
          </button>
        ))}
        ...
        <div className="p-5 rounded-xl border border-dashed border-gray-200 dark:border-gray-700 flex items-center justify-center">
          <p className="text-sm text-gray-400 dark:text-gray-500">{t('import.moreComingSoon')}</p>
        </div>
      </div>
      ...
      {activeType && (
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6">
          ...
          {result && (
            <div className="mt-4 p-4 bg-green-50 dark:bg-green-900/20 border border-green-200 rounded-lg">
              ...
              <div className="grid grid-cols-2 md:grid-cols-5 gap-3 text-sm">
                ...
                <div>
                  <p className="text-gray-500 dark:text-gray-400">{t('import.income')}</p>
                  <p className="font-semibold text-green-600">{result.income}</p>
                </div>
                <div>
                  <p className="text-gray-500 dark:text-gray-400">{t('import.skipped')}</p>
                  <p className="font-semibold text-gray-400 dark:text-gray-500">{result.skipped}</p>
                </div>
                <div>
          ...
          {error && (
            <div className="mt-4 p-4 bg-red-50 dark:bg-red-900/20 border border-red-200 rounded-lg flex items-start gap-2">
